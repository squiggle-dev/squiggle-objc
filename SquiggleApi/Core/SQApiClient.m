...
- (instancetype)initWithBaseURL:(NSURL *)url {
    ...
    if (self) {
        ...
        self.responseDeserializer = [[SQResponseDeserializer alloc] init];
        self.sanitizer = [[SQSanitizer alloc] init];
...
- (void) configureCacheReachibility {
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        reachabilityStatus = status;
        SQDebugLog(@"reachability changed to %@",AFStringFromNetworkReachabilityStatus(status));
        [SQApiClient setOfflineState:status == AFNetworkReachabilityStatusUnknown || status == AFNetworkReachabilityStatusNotReachable];
...
- (void) operationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        strongSelf.HTTPResponseHeaders = SQ__headerFieldsForResponse(response);
        if(!error) {
...
- (AFSecurityPolicy *) customSecurityPolicy {
    ...
    SQConfiguration *config = [self configuration];
...
- (SQConfiguration*) configuration {
    return [SQConfiguration sharedConfig];
}
